...
public static class BangumiApisExtensions
{
    ...
    public static PageCollection<SearchResponsedSubject> SearchSubjects(this IBangumiClient client, SearchSubjectsRequestBody requestBody, int? pageLimit = null, CancellationToken cancellationToken = default)
    {
        requestBody = requestBody.Clone();
        return new PageCollection<SearchResponsedSubject>(pageLimit, cancellationToken,
    ...
    /// <returns></returns>
    public static PageCollection<Subject> GetSubjects(this IBangumiClient client, GetSubjectsQueries queries, int? limit = null, CancellationToken cancellationToken = default)
    {
        queries = queries.Clone();
        return new PageCollection<Subject>(limit, cancellationToken,
            (lmt, ofs, token) => client.GetPagedSubjectsAsync(queries, lmt, ofs, token));
    }
